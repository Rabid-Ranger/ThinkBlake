(function(root,factory){
  ...
  function transformHtml(win,c,html,W){
    ...
    if(prefs.mode==='video'){
      ...
      const freshFlow=flowHtml(doc,prefs);
      ...
      const oldFlow=box.querySelector('.ar-flow'),freshFlow=flowHtml(doc,prefs);
      if(oldFlow)oldFlow.replaceWith(freshFlow);else box.prepend(freshFlow);
      ...
      const flow=box.querySelector('.au3-flow');flow?flow.after(intro):box.prepend(intro);

      const deep=box.querySelector('.ar-deep');
      if(deep){
        const h2=deep.querySelector('.ar-deep-head h2');if(h2)h2.textContent='Additional content-plan + business context';
        const p=deep.querySelector('.ar-deep-head p');if(p)p.textContent='These are the pieces not already shown in the main 90-day tables. Use them to understand older videos, what to make next, and business impact.';
        [...deep.querySelectorAll('.ar-sub')].forEach(sub=>{
          const title=(sub.querySelector('h3')?.textContent||'').trim();
          if(['Reach + viewing','Where the views came from','New + returning viewers · rolling 28 days'].includes(title))sub.remove();
        });
      }
    }
