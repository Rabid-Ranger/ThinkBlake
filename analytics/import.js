...
function collectionPrompt(c,hours=168,mode='update',now=new Date().toISOString()){
 ...
 if(mode==='audience'){
  ...
  const refs=(mode==='missing'?saved.filter(o=>o.coverage!=='exact'||studioCoreMetrics.some(k=>o.metrics?.[k]==null)).sort((a,b)=>b.publishedAt.localeCompare(a.publishedAt)).slice(0,5):saved);
