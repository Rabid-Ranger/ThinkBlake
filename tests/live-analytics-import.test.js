...
test('Studio prose and identical repeated JSON blocks import; conflicting repeated payloads do not',()=>{const packet=text([row(1)]);assert.equal(I.parse('Here you go:\n```json\n'+packet+'\n```\nCheck the report.',c,A.emptyStore()).added,1);assert.throws(()=>I.parse('Summary only',c,A.emptyStore()),/Nothing was saved/);assert.equal(I.parse('```json\n'+packet+'\n```\n```json\n'+packet+'\n```',c,A.emptyStore()).added,1);const changed=text([{...row(1),metrics:{...row(1).metrics,views:9999}}]);assert.throws(()=>I.parse(packet+'\n'+changed,c,A.emptyStore()),/multiple conflicting.*Nothing was saved/i);assert.throws(()=>I.parse('null',c,A.emptyStore()));});
