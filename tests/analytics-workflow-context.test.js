...
const read=(out,imp,ctrDelta,retDelta,opts={})=>({r:{comparisons:{
  ...
  ctr:{deltaPp:ctrDelta,current:opts.ctrCurrent??5,baseline:opts.ctrBase??5},
  retention30:{deltaPp:retDelta,current:opts.retCurrent??65,baseline:opts.retBase??65},
  apv:{deltaPp:opts.apvDelta??retDelta,current:opts.apvCurrent??45,baseline:opts.apvBase??45},
  avdSeconds:{multiple:opts.avdMultiple??1,current:opts.avdCurrent??180,baseline:opts.avdBase??180}
}}});
...
test('each diagnosis question answers with the metric that question asks for',()=>{
  ...
  try{
    ...
    assert.match(q.outcome.line,/creator normal/);
    assert.match(q.show.line,/creator normal/);
    assert.match(q.click.line,/CTR/);
    assert.match(q.click.line,/normal/);
...
test('soft metric clues do not automatically become the channel bottleneck',()=>{
  ...
  try{
    ...
    assert.match(q.click.label,/soft/i);
...
test('missing metric tells the coach exactly what is missing instead of generic Need data',()=>{
  ...
  try{
    ...
    const ADC={audienceRead:()=>({acquisitionBand:'unknown',loyaltyBand:'unknown',current:{},previous:{}}),overallRead:()=>({action:{job:'Keep intended job'},stages:[]})};
    const q=A.questionAnswers({},W,ADC);
    assert.match(q.click.label,/can’t answer/i);
    assert.match(q.click.line,/CTR/i);
    assert.match(q.click.next,/7-day CTR baseline/i);
