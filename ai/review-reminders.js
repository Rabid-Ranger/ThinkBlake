(() => {
  ...
  function impact(binding) {
    ...
    if (/^audience\./.test(value)) return { target: 'strategy', label: 'Audience changed', copy: 'Review the message, monthly focus and active video viewer decisions.' };
    ...
    if (/^analytics\.(?:_24h|_48h|_7d|_28d|sourceContext)/.test(value)) return { target: 'learn', label: 'New result evidence', copy: 'A learning can now be prepared from this checkpoint.' };
